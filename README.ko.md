...
현재 릴리스: Minecraft Java `1.20.1`용 `0.1.18`.
...
- 메인 장면과 bloom buffer는 LDR 경로를 유지합니다. HDR headroom 없이 local detail과 highlight rolloff만 사용합니다.
- 물 명도는 직접 태양광뿐 아니라 하늘/주변 장면 밝기도 따라가며, 밤에는 cap을 걸어 너무 밝게 떠 보이지 않게 했습니다.
- 밤에는 foliage PBR/normal 반응을 눌러 잎과 풀이 파란 점/선으로 깨져 보이는 현상을 줄였습니다.
- bloom은 bright extraction, downsample blur, upsample accumulation, final compositing으로 나뉘며, 비/밤/실내에서 과하게 번지지 않게 기본값을 잡았습니다.
- torch 같은 warm emissive source뿐 아니라 cyan/blue/magenta 네온 emissive source에도 screen-space RT local lighting을 적용합니다. 보이는 광원은 tracing하고, 보이지 않는 block light는 stable light-field fallback으로 처리합니다.
- `RT_LOCAL_SAMPLE_QUALITY`, `RT_LOCAL_NEON_STRENGTH`, `RT_LOCAL_NEON_SPILL`, `CYBER_NEON_SURFACE_GAIN`으로 RT tap 비용, 네온 강도, 젖은 표면 spill, 최종 emissive 재질 반응을 조절합니다.
...
2. `dist/Client-GLSL-Shaderpack-Lab-0.1.18-mc1.20.1.zip`을 `.minecraft/shaderpacks/`에 복사합니다.
...
- `low`: 비용이 큰 항목을 먼저 낮춥니다. shadow map 해상도/거리, SSAO 반경/강도, RT local light tracing 거리/step/tap quality, neon spill, bloom radius/GI, water SSR step/거리, rain SSR, fog/cloud 강도, bump relief, glass/metal reflection을 줄입니다. 또한 `composite6` reflected-geometry water pass를 끄고 final blend를 `0.00`으로 둡니다.
- `balanced`: 현재 shader source의 기본값과 생성형 PBR 맵 튜닝 기준입니다. vanilla 유리, 금속 블록, 얼음, 돌, 벽돌, 타일, polished material 반응, 네온 RT local light, 안정화된 water reflected-geometry pass, screen GI, leaf SSS, wet-weather scatter, bump-map relief를 볼 때 기준으로 사용합니다.
- `cinematic`: 같은 항목들을 시각 품질 쪽으로 올립니다. 4096 shadow, 160 block shadow distance, 더 강한 SSAO/screen GI, 더 넓은 네온 RT local light sampling, 큰 bloom/GI, 긴 water SSR/reflected-geometry tracing, 강한 fog/cloud/rain atmosphere, 더 강한 leaf SSS, 더 강한 bump relief, 더 강한 world-anchored water perturbation, 더 강하지만 height가 과하게 흔들리지 않는 material reflection을 사용합니다.
...
- cyan/blue/magenta emissive 픽셀이 기존 warm torch 경로와 함께 screen-space local color를 던지는 cyber-neon RT local lighting을 추가했습니다.
- `RT_LOCAL_SAMPLE_QUALITY`, `RT_LOCAL_NEON_STRENGTH`, `RT_LOCAL_NEON_SPILL`, `CYBER_NEON_SURFACE_GAIN` 설정을 추가하고, `low`, `balanced`, `cinematic` profile 값도 비용과 스타일 차이에 맞춰 조정했습니다.
- 비에 젖은 표면 reflection과 final emissive material finish가 새 render target 없이 local neon color를 더 분명하게 받도록 했습니다.

### 0.1.17

- `0.1.16`의 HDR buffer/tone mapping 실험을 제거하고 scene/bloom target을 제한된 LDR 경로로 되돌렸습니다.
- 물 명도가 직접 sun path만 보지 않고 sky/주변 장면 밝기도 따라가게 했습니다. 단, 밤에는 cap을 걸어 물이 과하게 밝게 뜨지 않도록 했습니다.
- 밤 시간대 foliage PBR normal/detail과 vegetation color lift를 낮춰 잎/풀이 파란 점과 선으로 깨지는 현상을 줄였습니다.
